...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, lst_ims=[]):
        if len(lst_ims) > 0:
            print(len(lst_ims))
            self.lst_ims = lst_ims
        ...
        if any(videos):
            ...
            self.cap = None
        assert self.nf > 0 or len(self.lst_ims) > 0, f'No images or videos found in {p}. ' \
    ...
    def __next__(self):
        if self.count == self.lst_ims: # or self.count == self.nf
            raise StopIteration

        img0 = self.lst_ims[self.count]
